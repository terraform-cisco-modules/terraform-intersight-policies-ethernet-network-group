...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-eth-vlgrp-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Ethernet Network Group Policy.",

	"VlanSettings": {
        "AllowedVlans": "1-5,10-99",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/fabric/EthNetworkGroupPolicies/%s", moid), expectedJSONTemplate, vars)
}
